...
class PaperController extends Controller
{
    ...
    public function savePaper(Request $request)
    {
        ...
        // Process Content (Univer Polymorphic)
        if (!empty($request->contentJson)) {
     ...
     * View Paper as PDF
     */
    public function viewAsPdf($id)
    {
        $paper = $this->paperRepository->getPaperById($id);
        if (!$paper) {
            return response()->json(['error' => 'Paper not found'], 404);
        }

        $html = $paper->contentHtmlSanitized;
        if (empty($html)) {
            $html = '<html><body><h3>No content available</h3></body></html>';
        }

        // Standard PDF styling for Papers
        $styledHtml = '
        <html>
        <head>
            <style>
                ...
                h1 { color: #333; }
                .paper-header { border-bottom: 2px solid #eee; margin-bottom: 30px; padding-bottom: 10px; }
        ...
        $filename = Str::slug($paper->name) . '.pdf';
