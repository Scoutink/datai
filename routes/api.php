...
Route::middleware(['auth'])->group(function () {
    ...
    Route::group(['middleware' => ['hasToken:ALL_DOCUMENTS_VIEW_DETAIL,ASSIGNED_DOCUMENTS_VIEW_DETAIL,DOCUMENT_PERMISSION_VIEW_DOCUMENT_PERMISSION']], function () {
        Route::get('/documentPermission/{id}', [DocumentPermissionController::class, 'getDocumentPermissionList']);
    });

    // --- Papers Module Routes ---
    
    // List & Detail
    Route::middleware('hasToken:ALL_PAPERS_VIEW_PAPERS')->group(function () {
        Route::get('/papers', [PaperController::class, 'getPapers']);
    });
    Route::get('/papers/assigned', [PaperController::class, 'getAssignedPapers']);
    Route::get('/papers/{id}/view-as-pdf', [PaperController::class, 'viewAsPdf']);
    Route::get('/papers/{id}', [PaperController::class, 'getPaper']);

    // CRUD
    Route::group(['middleware' => ['hasToken:ALL_PAPERS_CREATE_PAPER, MY_PAPERS_CREATE_PAPER']], function () {
        Route::post('/papers', [PaperController::class, 'savePaper']);
    });
    Route::group(['middleware' => ['hasToken:ALL_PAPERS_EDIT_PAPER, MY_PAPERS_EDIT_PAPER']], function () {
        Route::put('/papers/{id}', [PaperController::class, 'updatePaper']);
    });
    Route::group(['middleware' => ['hasToken:ALL_PAPERS_DELETE_PAPER, MY_PAPERS_DELETE_PAPER']], function () {
        Route::delete('/papers/{id}', [PaperController::class, 'deletePaper']);
    });

    // Permissions
    Route::middleware('hasToken:PAPER_PERMISSION_VIEW')->group(function () {
        Route::get('/papers/{id}/permissions', [PaperController::class, 'getPermissions']);
    });
    Route::middleware('hasToken:PAPER_PERMISSION_MANAGE')->group(function () {
        Route::post('/papers/permissions/role', [PaperController::class, 'addRolePermission']);
        Route::post('/papers/permissions/user', [PaperController::class, 'addUserPermission']);
        Route::delete('/papers/permissions/role/{id}', [PaperController::class, 'deleteRolePermission']);
        Route::delete('/papers/permissions/user/{id}', [PaperController::class, 'deleteUserPermission']);
    });

    // History / Versions
    Route::middleware('hasToken:PAPER_HISTORY_VIEW')->group(function () {
        Route::get('/papers/{id}/versions', [PaperController::class, 'getVersions']);
    });
    Route::middleware('hasToken:PAPER_HISTORY_RESTORE')->group(function () {
        Route::post('/papers/versions/{id}/restore', [PaperController::class, 'restoreVersion']);
    });

    // Comments
    Route::get('/papers/{id}/comments', [PaperController::class, 'getComments']);
    Route::post('/papers/comments', [PaperController::class, 'addComment']);
    Route::delete('/papers/comments/{id}', [PaperController::class, 'deleteComment']);

    // Audit
    Route::middleware('hasToken:PAPER_AUDIT_VIEW')->group(function () {
        Route::get('/papers/{id}/audit', [PaperController::class, 'getAuditTrails']);
    });

    // Shareable Links
    Route::middleware('hasToken:PAPER_SHARE_MANAGE')->group(function () {
        Route::get('/papers/{id}/share-link', [PaperController::class, 'getShareLink']);
        Route::post('/papers/share-link', [PaperController::class, 'saveShareLink']);
        Route::delete('/papers/share-link/{id}', [PaperController::class, 'deleteShareLink']);
    });

    // Editor.js Assets
    Route::post('/papers/assets/upload', [PaperController::class, 'uploadAsset']);
    Route::get('/papers/assets/fetch-url', [PaperController::class, 'fetchUrl']);

    // Sheets Polymorphic Routes
    Route::group(['middleware' => ['hasToken:ALL_PAPERS_VIEW_DETAIL,MY_PAPERS_VIEW_DETAIL']], function () {
        Route::get('/papers/{paperId}/sheets/columns', [\App\Http\Controllers\Api\SheetController::class, 'getColumns']);
        Route::get('/papers/{paperId}/sheets/rows', [\App\Http\Controllers\Api\SheetController::class, 'getRows']);
    });

    Route::group(['middleware' => ['hasToken:ALL_PAPERS_EDIT_PAPER,MY_PAPERS_EDIT_PAPER']], function () {
        Route::post('/papers/{paperId}/sheets/columns', [\App\Http\Controllers\Api\SheetController::class, 'storeColumn']);
        Route::patch('/sheets/columns/{id}', [\App\Http\Controllers\Api\SheetController::class, 'updateColumn']);
        Route::delete('/sheets/columns/{id}', [\App\Http\Controllers\Api\SheetController::class, 'deleteColumn']);

        Route::post('/papers/{paperId}/sheets/rows', [\App\Http\Controllers\Api\SheetController::class, 'storeRow']);
        Route::patch('/sheets/rows/{rowId}', [\App\Http\Controllers\Api\SheetController::class, 'updateRow']);
        Route::delete('/sheets/rows/{rowId}', [\App\Http\Controllers\Api\SheetController::class, 'deleteRow']);
    });

    // --- End Papers Module Routes ---
    Route::group(['middleware' => ['hasToken:ALL_DOCUMENTS_MANAGE_COMMENT,ASSIGNED_DOCUMENTS_MANAGE_COMMENT,ALL_DOCUMENTS_VIEW_DETAIL,ASSIGNED_DOCUMENTS_VIEW_DETAIL']], function () {
    ...
    Route::group(['middleware' => ['hasToken:ALL_DOCUMENTS_SHARE_DOCUMENT,ASSIGNED_DOCUMENTS_SHARE_DOCUMENT,ALL_DOCUMENTS_VIEW_DETAIL,ASSIGNED_DOCUMENTS_VIEW_DETAIL']], function () {
...
// ============================================
// INTERACTIVE MODULE (H5P)
// ============================================
Route::group(['prefix' => 'interactive', 'middleware' => ['auth', 'hasToken:MANAGE_INTERACTIVE_CONTENT']], function () {
    Route::get('/content', [InteractiveController::class, 'index']);
    Route::get('/content/{id}', [InteractiveController::class, 'show']);
    Route::get('/libraries', [InteractiveController::class, 'libraries']);

    // H5P Editor Bridge
    Route::get('editor/settings/{id?}', [App\Http\Controllers\Api\InteractiveEditorController::class, 'settings']);
    Route::match(['GET', 'POST'], 'editor/ajax', [App\Http\Controllers\Api\InteractiveEditorController::class, 'ajax']);
    Route::post('editor/store', [App\Http\Controllers\Api\InteractiveEditorController::class, 'store']);
});
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::middleware('hasToken:BOARDS_EDIT_BOARD')->group(function () {
        Route::put('/boards/{id}', [BoardController::class, 'update']);
        Route::post('/boards/{boardId}/columns', [BoardController::class, 'storeColumn']);
        Route::put('/boards/{boardId}/columns/reorder', [BoardController::class, 'reorderColumns']);
        Route::put('/columns/{id}', [BoardController::class, 'updateColumn']);
        Route::delete('/columns/{id}', [BoardController::class, 'destroyColumn']);
    ...
    Route::middleware('hasToken:BOARDS_DELETE_BOARD')->group(function () {
        Route::delete('/boards/{id}', [BoardController::class, 'destroy']);
    });

    Route::middleware('hasToken:BOARDS_MANAGE_CARDS')->group(function () {
        Route::post('/columns/{columnId}/cards', [BoardController::class, 'storeCard']);
        Route::get('/cards/{id}', [BoardController::class, 'showCard']);
        Route::put('/cards/{id}', [BoardController::class, 'updateCard']);
        Route::delete('/cards/{id}', [BoardController::class, 'destroyCard']);
        Route::put('/cards/{id}/move', [BoardController::class, 'moveCard']);
        Route::put('/cards/{id}/archive', [BoardController::class, 'archiveCard']);
        Route::post('/cards/{cardId}/assignees', [BoardController::class, 'addAssignee']);
        Route::delete('/cards/{cardId}/assignees/{userId}', [BoardController::class, 'removeAssignee']);
        Route::post('/cards/{cardId}/attachments', [BoardController::class, 'attachDocument']);
        Route::delete('/cards/{cardId}/attachments/{documentId}', [BoardController::class, 'detachDocument']);
        Route::post('/cards/{cardId}/labels', [BoardController::class, 'addCardLabel']);
        Route::delete('/cards/{cardId}/labels/{labelId}', [BoardController::class, 'removeCardLabel']);
        Route::post('/cards/{cardId}/tags', [BoardController::class, 'addCardTag']);
        Route::delete('/cards/{cardId}/tags/{tagId}', [BoardController::class, 'removeCardTag']);
        Route::post('/cards/{cardId}/followers', [BoardController::class, 'addCardFollower']);
        Route::delete('/cards/{cardId}/followers/{userId}', [BoardController::class, 'removeCardFollower']);
        Route::get('/cards/{cardId}/activities', [BoardController::class, 'getCardActivities']);
        Route::post('/cards/{cardId}/comments', [BoardController::class, 'storeCardActivity']);
        Route::get('/boards/{boardId}/archived-cards', [BoardController::class, 'getArchivedCards']);
        Route::put('/cards/{id}/restore', [BoardController::class, 'restoreCard']);

        // Checklists
        Route::post('cards/{cardId}/checklists', [BoardController::class, 'storeChecklist']);
        Route::delete('checklists/{id}', [BoardController::class, 'destroyChecklist']);
        Route::post('checklists/{checklistId}/items', [BoardController::class, 'storeChecklistItem']);
        Route::put('checklist-items/{id}', [BoardController::class, 'updateChecklistItem']);
        ...
        Route::post('/cards/{cardId}/milestones/sync', [\App\Http\Controllers\BoardMilestoneController::class, 'syncCardMilestones']);
    });
});

// DEBUG: Temporary H5P Probe
if (file_exists(base_path('routes/h5p_debug.php'))) {
    require_once base_path('routes/h5p_debug.php');
}

// DEBUG: Controller Test Probe
if (file_exists(base_path('routes/h5p_controller_test.php'))) {
    require_once base_path('routes/h5p_controller_test.php');
}

// H5P Shim Routes (Required by LaravelH5p package to avoid RouteNotFoundException)
Route::group(['prefix' => 'h5p-shim', 'middleware' => ['auth:api']], function () {
    Route::any('ajax', function() { return response()->json([]); })->name('h5p.ajax');
    Route::any('ajax/finish', function() { return response()->json([]); })->name('h5p.ajax.finish');
    Route::any('ajax/content-user-data', function() { return response()->json([]); })->name('h5p.ajax.content-user-data');
    Route::any('ajax/rebuild-cache', function() { return response()->json([]); })->name('h5p.ajax.rebuild-cache');
    
    Route::any('index', function() { return response()->json([]); })->name('h5p.index');
    Route::any('export/{id}', function() { return response()->json([]); })->name('h5p.export');
    Route::any('embed/{id}', function() { return response()->json([]); })->name('h5p.embed');
    
    Route::any('library', function() { return response()->json([]); })->name('h5p.library.index');
    Route::any('library/store', function() { return response()->json([]); })->name('h5p.library.store');
    Route::any('library/clear', function() { return response()->json([]); })->name('h5p.library.clear');
    Route::any('library/restrict', function() { return response()->json([]); })->name('h5p.library.restrict');
    Route::any('library/destroy', function() { return response()->json([]); })->name('h5p.library.destroy');
    
    Route::any('content/create', function() { return response()->json([]); })->name('h5p.create');
    Route::any('content/{id}', function() { return response()->json([]); })->name('h5p.show');
    Route::any('content/{id}/edit', function() { return response()->json([]); })->name('h5p.edit');
    Route::any('content/{id}/destroy', function() { return response()->json([]); })->name('h5p.destroy');
});
